    ...
    <div className="min-h-screen bg-indigo-50 px-6 py-10">
      <div className="max-w-4xl mx-auto bg-white shadow-lg rounded-2xl p-8">
        ...
        <p className="mb-8 text-lg text-center font-medium text-gray-700">
          Order Total: <span className="text-indigo-600 font-bold">₹{total}</span>
        </p>

        {/* Saved Addresses */}
        <div className="mb-8 bg-indigo-50 p-5 rounded-xl shadow-sm">
          <h3 className="text-lg font-semibold text-indigo-800 mb-4">Select Shipping Address</h3>
          {addresses.length === 0 ? (
            <p className="text-gray-600">No saved addresses. Add one below.</p>
          ) : (
            <ul className="space-y-3">
              {addresses.map((addr, idx) => (
                <li key={idx} className="border border-indigo-100 p-3 rounded-lg bg-white shadow-sm flex items-start hover:shadow-md transition">
                  <input
                    type="radio"
                    name="selectedAddress"
                    className="mt-1 mr-3 text-indigo-600 focus:ring-indigo-500"
                    checked={selectedAddress && selectedAddress.street === addr.street}
        ...
        {/* Add New Address */}
        <div className="mb-8 bg-indigo-50 p-5 rounded-xl shadow-sm">
          <h3 className="text-lg font-semibold text-indigo-800 mb-4">Add New Address</h3>
        ...
        {/* Coupon Section */}
        <div className="mb-8 bg-indigo-50 p-5 rounded-xl shadow-sm">
          <h3 className="text-lg font-semibold text-indigo-800 mb-3">Apply Coupon</h3>
          <div className="flex">
            <input
              type="text"
              value={couponCode}
          ...
          {appliedCoupon && (
            <div className="mt-4 text-green-600 font-medium">
              ...
              <p>New Total: <span className="font-bold">₹{appliedCoupon.totalAfterDiscount}</span></p>
            </div>
          )}

          {/* Available Coupons */}
          <div className="mt-6">
            <h4 className="text-md font-semibold mb-2 text-gray-800">Available Coupons</h4>
            {availableCoupons.length === 0 ? (
              <p className="text-gray-600">No coupons available</p>
            ) : (
              <div className="grid gap-3">
                {availableCoupons.map((c) => (
                  <div key={c._id} className="border border-indigo-200 rounded-lg p-3 flex justify-between items-center bg-white shadow-sm hover:shadow-md transition">
                    <div>
